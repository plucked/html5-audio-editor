function AudioLayerSequenceEditor(elementContext)
{
    ...
    this.del = function del(processLinks)
    {
        ...
        var end = (this.selectionEnd < 0) ? 0 : (this.selectionEnd >= this.audioSequenceReference.data.length) ? this.audioSequenceReference.data.length - 1 : this.selectionEnd;
				...
				this.undoHistory.push(this.audioSequenceReference.clone(0, this.audioSequenceReference.data.length));
				this.redoHistory = []; // reset redo history
		...
		this.undo = function undo(processLinks)
		{
			if (this.undoHistory.length == 0) {
				console.warn('no undo history');
				return;
			}

			var historySequence = this.undoHistory[this.undoHistory.length - 1];
			this.selectionStart = 0;
			this.selectionEnd = 0;

			this.redoHistory.push(this.audioSequenceReference.clone(0, this.audioSequenceReference.data.length));
			this.audioSequenceReference.change(historySequence);
			...
			if (processLinks !== undefined && processLinks === true)
			{
					for (var i = 0; i < this.linkedEditors.length; ++i)
					{
							this.linkedEditors[i].undo(false);
					}
			}

		}

		this.redo = function redo(processLinks)
		{
			...
			var historySequence = this.redoHistory[this.redoHistory.length - 1];
			this.selectionStart = 0;
			this.selectionEnd = 0;
			...
			if (processLinks !== undefined && processLinks === true)
			{
					for (var i = 0; i < this.linkedEditors.length; ++i)
					{
							this.linkedEditors[i].redo(false);
					}
			}
...
window.addEventListener("keydown", function(e)
{
    ...
    if (e.keyCode == 46) // Delete
    {
		...
		{
			focusOnAudioLayerSequenceEditor.undo(true);
		}

		if (e.ctrlKey && e.shiftKey && e.keyCode == 90) // ctrl + shift + z
		{
			focusOnAudioLayerSequenceEditor.redo(true);
		}
